...
POINTS_COLUMN_NAMES = ['latitude', 'longitude', 'elevation', 'distance', 'watts',
                       'time', 'heart_rate', 'cadence', 'speed', 'lap']

# The names of the columns we will use in our laps DataFrame
LAPS_COLUMN_NAMES = ['number', 'start_time', 'distance',
                     'total_time', 'max_speed', 'max_hr', 'avg_hr']
...
def get_tcx_point_data(point: lxml.etree._Element) -> Optional[Dict[str, Union[float, int, str, datetime]]]:
    ...
    if position is None:
        ...
        data['latitude'] = float(position.find(
            'ns:LatitudeDegrees', NAMESPACES).text)
        data['longitude'] = float(position.find(
            'ns:LongitudeDegrees', NAMESPACES).text)

    time_str = point.find('ns:Time', NAMESPACES).text
    data['time'] = dp.parse(time_str)

    distanceElem = point.find('ns:DistanceMeters', NAMESPACES)
    if distanceElem is not None:
        data['distance'] = float(distanceElem.text)
    ...
    powerElem = point.find('.//ns3:Watts', NAMESPACES)
    if powerElem is not None:
        data['watts'] = float(powerElem.text)
